...
namespace ApiPetShop.Domain
{
    public class PetRepository(DbConnectionContext db, IMapper mapper) : IPetRepository
    {
        ...
        public async Task<List<DateTime>> GetScheduledTime(DateTime date) => await db.PetServices.AsNoTracking().Where(x => x.ScheduledDate.Day == date.Day && x.ScheduledDate.Month == date.Month && x.ScheduledDate.Year == date.Year).Select(x => x.ScheduledDate).ToListAsync() ?? [];
