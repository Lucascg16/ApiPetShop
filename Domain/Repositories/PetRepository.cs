...
namespace ApiPetShop.Domain
{
    public class PetRepository(DbConnectionContext db, IMapper mapper) : IPetRepository
    {
        ...
        public async Task<List<ServiceListDto>> GetByDate(DateTime date) => mapper.Map<List<ServiceListDto>>(await db.PetServices.AsNoTracking().Where(x => x.ScheduledDate.DayOfYear == date.DayOfYear && x.ScheduledDate.Year == date.Year).OrderBy(x => x.ScheduledDate).ToListAsync());
